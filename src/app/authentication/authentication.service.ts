...
export class AuthenticationService extends BaseService<User> {
    ...
    constructor(private http: HttpClient,
        ...
        super( http, '');
        this.baseURL = 'https://rentals.robisignals.com/backend/api/v1';
    }
    ...
    login(username: string, password: string) {
        return this.http.post<any>(`${env.baseURL}/login`, {email: username, password})
            .pipe(map((user) => {
     ...
     */
    logout() {
        return this.http.post<any>(`${env.baseURL}/logout`, {})
            .pipe(map((data) => {
            }));
    }
